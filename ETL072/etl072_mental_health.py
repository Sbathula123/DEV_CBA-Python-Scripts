...
#cursor=generic_module.create_sql_cursor(server_name='azsyddwh02',database_name='db-au-stage')
...
from
    [db-au-cba]..vClaimTagsFeeder f WITH (NOLOCK)
   ...
   -- cl.ClaimNo % 2 = 0 and
    ...
    (
        select
            null
        from
            [db-au-cba]..clmClaimTags t  with(index(idx_clmClaimTags_ClaimKey) NOLOCK)
        where
...
order by
    cl.CreateDate desc"""
#cursor.execute(vsql)
claim_df=generic_module.execute_sql_and_return_result(server_name='azsyddwh02',database_name='db-au-stage',sql=vsql)
#print(claim_df)
...
for index,row in claim_df.iterrows():
    try:
        ...
        update_sql="""
                    merge into [db-au-cba].dbo.clmClaimTags t
                    using 
                        (
                            select 
        ...
        #print(update_sql)
        #sys.exit()
        generic_module.execute_sql_dml(server_name='azsyddwh02',database_name='db-au-stage',sql=update_sql)
