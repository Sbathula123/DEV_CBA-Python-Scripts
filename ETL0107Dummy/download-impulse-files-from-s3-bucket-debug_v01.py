    ...
    for ele in bucket_object_keys:
        ...
        if re.match(r"^(\2023-\11-\3\/)$", ele): 
            try:
                ...
                pass
        """
        #pattern = r"\d{4}-\d{2}-\d{1}"
        #ele_dir = re.search(r"^2023-11-3/", ele).group()
        #try:
        #    ele_dir = re.match(r"\d{4}-\d{2}-\d{2}\/", ele).group()
        #except AttributeError:
        #    ele_dir = re.match(r"\d{4}-\d{2}-\d{2}\/", ele)
        #ele_dir = re.search(r"\d{4}-\d{2}-\d{1}\/", ele).group()
        ele_dt = datetime.datetime.strptime('2023-11-3', "%Y-%m-%d") # parses YYYY-MM-DD format into datetime object
        
        if ('2023-11-3', ele_dt) not in directory_date_pairs:
            directory_date_pairs.append(('2023-11-3', ele_dt))           
