...
try:
    ...
    if not generic_module.is_step_complete(generic_module.module_name,1):
        ...
        cursor_ref.execute("exec [db-au-stage]..syssp_createbatch @SubjectArea = 'CDG Quote Bigquery CBA',@StartDate = '"+generic_module.start_date+"',@EndDate = '"+generic_module.end_date+"'");
        cursor_ref.commit()
...
--        '' CRMUserKey,
    ...
    if not generic_module.is_step_complete(generic_module.module_name,53):
        cursor_ref=generic_module.create_sql_cursor('uldwh02','db-au-stage')
        cursor_ref.execute("exec [db-au-stage].dbo.etlsp_ETL115_RetrievePenQuoteSummaryCBADelta");
        cursor_ref.commit()
    ...
    vsql="""declare
            ...
            exec [db-au-stage]..syssp_getrunningbatch
                @SubjectArea = 'CDG Quote Bigquery CBA',
                @BatchID = @batchid out,
                @StartDate = @start out,
                @EndDate = @end out
                
            exec [db-au-stage]..[etlsp_ETL115_factQuoteSummaryCBA] 
