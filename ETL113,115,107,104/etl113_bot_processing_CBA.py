...
#sys.path.append(r'\\azsyddwh03\ETL\Python Scripts')
...
--                qc.QuoteCountryKey = q.QuoteCountryKey
  ...
  select 
    ...
    if not generic_module.is_step_complete(generic_module.module_name,56):
        cursor_ref=generic_module.create_sql_cursor('azsyddwh03','db-au-stage')#processed for last 14 days.
        """vsql=--declare 
               -- @start date
                
           -- select 
                @start = dateadd(day, -14, cast('+generic_module.start_date+' as date))"""
        vsql="""exec [db-au-stage].dbo.etlsp_ETL113_factQuoteSummaryBot   @DateRange = '_User Defined',
